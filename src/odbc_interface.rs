...
pub fn entry_point() {
    ...
    match connect(&mut recordset, String::from("DSN=odbc2;"), String::from("select * from results.csv;")) {
        Ok(()) => println!("Success"),
        Err(diag) => { println!("Error: {}", diag); return; },
    }
    ...
    for ROW in recordset {
		for CELL in &ROW {
			...
			if CELL != &ROW[ROW.len() - 1] { payload.push_str(", "); }
		}
		payload.push_str("\n");
    }
...
pub fn connect(recordset: &mut Vec<Vec<String>>, _dsn: String, sql_text: String) -> std::result::Result<(), DiagnosticRecord> {
	...
	let conn = env.connect_with_connection_string("Driver=Microsoft Access Text Driver (*.txt, *.csv);Dbq=D:\\;Extensions=asc,csv,tab,txt;")?;

    execute_statement(&conn, recordset, sql_text)
}

fn execute_statement<'env>(conn: &Connection<'env, AutocommitOn>, recordset: &mut Vec<Vec<String>>, sql_text: String) -> Result<()> {
